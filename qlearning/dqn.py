...
class ExperienceBuffer:
    def __init__(self, capacity):
        self.buffer = collections.deque(maxlen=capacity)  # N.B: when the maximum length is reached, the leftmost element is automaitcally poped
    ...
    def sample(self, batch_size):
        ...
        return np.array(states), np.array(actions), np.array(rewards, dtype=np.float32), \
               np.array(dones, dtype=np.int32), np.array(next_states)
...
class Agent:
    ...
    def play_step(self, net, epsilon=0.0, device='cpu'):
        ...
        self.exp_buffer.append(Experience(state=self.state, action=action, reward=reward, done=is_done,
                                          new_state=new_state))
    ...
    while True:
        ...
        if reward is not None:
            ...
            if mean_reward > args.reward:
                print('Problem solved in %d frames!' %frame_idx)
                break
